...
const OrderTable: FC = async () => {
  ...
  // duruma bağlı renk döndüren fonksiyon
      ...
      <tbody>
        {orders.map((order) => (
          <tr key={order.id}>
            ...
            <td>
              <p
                className={`${getColor(order.status)} text-white text-shadow-2xs py-1 px-2 rounded-lg shadow text-center w-full text-nowrap max-w-40 mx-auto`}
              >
                {order.status}
              </p>
            </td>
