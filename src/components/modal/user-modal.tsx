...
const UserModal:FC<Props> = async ({userId}) => {
    // apiden kullancıı deyatları alm
    const user = await getUser(userId);

    //ekrana basılması gerekn değerleri bir dizi haline getirmek
     const fields = [
    {
      label: "Eposta :",
      value: user.email,
  ...
  return (
    <div className="absolute bg-black-10 backdrop-blur-[3px] inset-0 grid place-items-center p-10">
      <div className="bg-white rounded-lg shadow py-6 px-10 pb-14 w-full max-w-md">
        {/* modalı kapat */}
        <div className="flex justify-end items-center">
            <Link href="/users" className="button hover:bg-red-200">
            <CgClose className="text-2xl text-red-400"/>
            </Link>
        </div>
